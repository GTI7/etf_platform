...
def test_get_etf_by_ticker_round_trip(conn: sqlite3.Connection) -> None:
    insert_calendar(
        conn,
        Calendar(
            calendar_id=CALENDAR_ID,
    ...
    assert get_etf_by_ticker(conn, "SPY") == etf


def test_get_etf_by_ticker_returns_none_when_missing(conn: sqlite3.Connection) -> None:
    assert get_etf_by_ticker(conn, "DOES-NOT-EXIST") is None

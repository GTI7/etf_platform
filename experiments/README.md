...
A second **research analysis script**. Where `validate_scoring_signal.py`
(above) reports a single descriptive top-vs-bottom spread, this script
asks the sharper question that finding could not answer on its own:

> Is REFERENCE v1's ranking -- or either of its two dimensions --
> distinguishable from chance, once tested against a randomization null
> and checked for robustness against the sample's own serial
> dependence? Or is the observed spread indistinguishable from noise?

It reuses `generate_ranked_etf_report()`,
`daily_etf_universe_update.run(session_date=...)`, and
`validate_scoring_signal.py`'s own `forward_return()` /
`_resolve_close()` unmodified -- no duplicate data-access or
return-calculation logic. Everything new is pure statistics (Spearman
rank correlation, permutation testing, block bootstrap, Holm-Bonferroni
correction), implemented in the standard library only, no third-party
statistics package.

Five statistics are computed independently every run: MOMENTUM's daily
cross-sectional IC, VALUE's IC, the raw (unnormalized) blend's IC, a
blend IC after this script's own percentile-rank normalization, and the
top-vs-bottom bucket spread. Each gets its own permutation null (a
within-date reassignment of that statistic's own ranking, forward
returns held fixed -- MOMENTUM's shuffle never leaks into VALUE's test
or vice versa) and its own block-bootstrap confidence interval at three
block lengths (20 days is the primary result; 40 and 60 days are
robustness diagnostics only). A statistic is only treated as
significant if it survives Holm-Bonferroni correction across all five
tests *and* its bootstrap CI excludes zero at all three block lengths
-- surviving permutation testing alone is not sufficient, precisely
because permutation nulls discard the serial dependence real daily
rankings and forward returns have, while the block bootstrap preserves
it.

Requires the same real price depth as `validate_scoring_signal.py`
(`backfill_price_history.py` must already have populated the requested
period plus the forward-return horizon beyond it).
...
It prints the same kind of factual, disclaimer-terminated report as
`validate_scoring_signal.py`, plus a machine-readable
`reference_v1_significance_report.json` at the repo root (gitignored --
generated output, not source, regenerated by every run). The report's
final `decision` is always exactly one of `BUILD_V2_JUSTIFIED` or
`ARCHIVE`, reasoned mechanically from the significance/robustness rules
above, never from judgment calls made at read time. As with every
script in this directory: an `ARCHIVE` or `BUILD_V2_JUSTIFIED` verdict
is a statement about this sample only -- it does not confirm, validate,
or prove predictive value, does not account for transaction costs, and
cannot speak to a different market regime than the one observed. No
recommendation, no ranking judgment, no investment advice. No new
dependency.
